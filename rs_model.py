...
class RSModel(Model):
    ...
    def loss_fn_rmse(self, predictions, labels):
        ...
        loss += self.keyword_embedding.losses
        loss += self.user_embedding.losses + self.item_embedding.losses
        loss += self.bias_u.losses + self.bias_i.losses
        return loss
...
def get_val_rmse(rs_model, val_dataset):
    ...
    return np.sqrt(np.mean((all_predictions - all_ratings) ** 2))
...
#         train_dataset.shuffle()
        ...
        print('epoch: ', epoch, '. load data time: ', load_data_time, '. train time: ', train_time, '. train loss: ', train_loss.numpy() / (train_dataset.num_batch))
        if epoch % 2 == 0:
